...
def embedding(encoder_model, dataloader):
    encoder_model.eval()
    x = []
...
def main():
    ...
    model_save_dir="model/GraphCL_checkpoint/GraphCL_200.pt"
    embedding_save_path="model/GraphCL_embedding.pt"

    encoder_model=torch.load(model_save_dir, map_location=device)


    x = embedding(encoder_model, dataloader)
