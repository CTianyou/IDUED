...
class IncrementalSTDBSCAN:
    ...
    def __init__(self, eps=1,spatial_eps=1, min_pts=5, metric='minkowski', p=2):
        self.eps = eps
        ...
        self._objects = Objects(self.eps,self.spatial_eps, self.metric, self.p)
        self._inserter = Inserter(self.eps, self.spatial_eps,self.min_pts, self._objects)
        self._deleter = Deleter(self.eps, self.spatial_eps,self.min_pts, self._objects)

    def insert(self, X, points):
        ...
        """
        X,points = input_check(X, points)

        for value,point in tqdm(zip(X,points)):
            self._inserter.insert(value,point)

        return self

    def delete(self, X, points):
        ...
        """
        X = input_check(X, points)

        for ix, (value,point) in enumerate(zip(X,points)):
            obj = self._objects.get_object(value,point)
    ...
    def get_cluster_labels(self, X,points):
